...
class ProtectorState(Enum):
	...
	CAPTURED = 5                            # Protector has captured hunter whether treasure is hunted or not.
	DEAD = 6                                # Protector has hit a block.
	LOST = 7                                # Protector has missed the hunter and hunter is at safety with treasure hunted.
...
class Protector(IntelligentPlayer):
	...
	def deduct_next_move(self, hunter, treasure, shelter, effective_distance):
		...
		if self._general_protection_state == GeneralProtectionState.PROTECTING:
			...
			if self._detailed_protection_state == DetailedProtectingState.DIRECT_RESCUING:
				...
				shelter_weight, protector_weight = self.calculate_shelter_based_weights(hunter_shelter_distance)
				self._number_of_not_in_sight_chasing += 1
			...
			# Treasure protector guide vector
			if hunter_weight > 0:
				hunter_weights = self.find_other_player_move_vectors(hunter_move_vector, protector_weight)
			else:
				hunter_weights = np.zeros(self._number_of_feasible_moving_vectors)
	...
	def shall_we_go_on(self):
		...
		if self._protector_state != ProtectorState.PLAYING:
			return False
