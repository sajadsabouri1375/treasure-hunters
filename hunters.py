...
class GeneralHuntingState(Enum):
    ...
    SHELTER_WITH_TREASURE = 3           # Hunter has captured the treasure and is returning to shelter
...
class HunterState(Enum):
    ...
    CAPTURED = 2                        # Hunter is captured by the protector (whether before or after treasure is hunted).
    DEAD = 3                            # Hunter has hit a block.
    SAFE = 4                            # Hunter has returned to shelter safely without being captured.
    SAFE_WITH_TREASURE = 5              # Hunter has captured treasure and returned to shelter safely without being captured.
...
class Hunter(IntelligentPlayer):
    ...
    def get_hunter_state_string(self):
        ...
        elif self._hunter_state == HunterState.SAFE_WITH_TREASURE:
            return f'{Fore.LIGHTGREEN_EX}Safe With Treasure{Fore.RESET}'
    
    def get_hunting_general_state(self):
        return self._general_hunting_state
    
    def get_hunting_detailed_state(self):
        return self._detailed_hunting_state
    
    def set_hunting_state(self, new_hunting_state):
        
        if self._general_hunting_state != GeneralHuntingState.SHELTER_WITH_TREASURE:
            self._general_hunting_state = new_hunting_state
    ...
    def deduct_next_move(self, protector, treasure, shelter, effective_distance):
        ...
        if self._general_hunting_state == GeneralHuntingState.SHELTER or self._general_hunting_state == GeneralHuntingState.SHELTER_WITH_TREASURE:
    ...
    def update_hunting_state(self, protector, treasure, shelter, effective_distance):
        ...
        if self._general_hunting_state == GeneralHuntingState.HUNTING:
            if self.did_you_capture_treasure(treasure, effective_distance):
                self._general_hunting_state = GeneralHuntingState.SHELTER_WITH_TREASURE
        ...
        elif self._general_hunting_state == GeneralHuntingState.SHELTER or self._general_hunting_state == GeneralHuntingState.SHELTER_WITH_TREASURE:
    ...
    def update_hunter_state(self, protector, shelter, effective_distance):
        ...
        if self._general_hunting_state == GeneralHuntingState.SHELTER_WITH_TREASURE:
            if self.did_you_make_it_to_shelter(shelter, effective_distance):
                self._hunter_state = HunterState.SAFE_WITH_TREASURE
                return  
    ...
    def shall_we_go_on(self):
        ...
        return self._hunter_state == HunterState.PLAYING and self._is_active